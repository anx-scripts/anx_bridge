...
formatTable = function(t, indent)
    ...
    for k, v in pairs(t) do
        table.insert(
            lines,
            string.format("%s%s = %s,", padInner, formatKey(k), formatValue(v, indent))
        )
    end
...
shared.log = function(message, level)
    level = level or "info"
    local color = LOG_COLORS[level] or "^7"
    print(string.format("%s[%s]^7 %s", color, tostring(level):upper(), tostring(message)))
end
